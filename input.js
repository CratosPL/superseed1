...
function mousePressed() {
  ...
  if (
    ...
    connectionError = null;
      ...
      // Obsługa kliknięcia w "Technical Info"
      let techInfoX = GAME_WIDTH / 2 - 60;
      let techInfoY = GAME_HEIGHT - 50;
      let techInfoWidth = 120;
      let techInfoHeight = 20;
      if (
        adjustedMouseX >= techInfoX &&
        adjustedMouseX <= techInfoX + techInfoWidth &&
        adjustedMouseY >= techInfoY &&
        adjustedMouseY <= techInfoY + techInfoHeight
      ) {
        showTechnicalInfo = !showTechnicalInfo;
      }
  
      // Obsługa przycisku "Close" – nowa pozycja
      if (showTechnicalInfo) {
        let closeButtonX = GAME_WIDTH / 2 - 40;
        let closeButtonY = GAME_HEIGHT / 2 + 302; // Nowa pozycja zgodna z draw()
        let closeButtonWidth = 80;
        let closeButtonHeight = 25;
        if (
          adjustedMouseX >= closeButtonX &&
          adjustedMouseX <= closeButtonX + closeButtonWidth &&
          adjustedMouseY >= closeButtonY &&
          adjustedMouseY <= closeButtonY + closeButtonHeight
        ) {
